...
type Iteratorer interface {
	...
	Set(e interface{}) (b bool) //将该迭代器的当前指向元素设为e
	Index() (idx int)           //返回该迭代器指向元素的下标
...
//@receiver		nil
//@param    	data		[]interface{}	迭代器所承载的元素集合
...
//@description
//		以Iterator迭代器指针做接收者
//		将该迭代器当前下标所指元素设为预设元素e，修改成功返回true否则返回false
//		若迭代器为nil则直接结束并返回false
//		若该迭代器下标在元素集合范围内则进行修改并返回true
//		若该迭代器下标不在元素集合范围内，则不进行修改并返回false
//@author     	hlccd		2021-07-1
//@receiver		i			*Iterator		迭代器指针
//@param    	e			interface{}		预设元素e
//@return    	b			bool			修改成功？
func (i *Iterator) Set(e interface{}) (b bool) {
	if i == nil {
		//迭代器为nil则直接结束并返回false
		return false
	}
	if i.index >= 0 && i.index < len(i.data) {
		//该迭代器下标在元素集合范围内，进行修改并返回true
		i.data[i.index] = e
		return true
	} else {
		//该迭代器下标不在元素集合范围内，不进行修改并返回false
		return false
	}
}
...
//		以Iterator迭代器指针做接收者
//		返回迭代器当前下标
//		若迭代器为nil或元素集合为空，返回-1
//		否则返回迭代器当前下标
//@author     	hlccd		2021-07-1
//@receiver		i			*Iterator		迭代器指针
//@param    	nil
//@return    	idx			int				迭代器下标
func (i *Iterator) Index() (idx int) {
	if i == nil {
		//迭代器为nil，返回-1
		return -1
	}
